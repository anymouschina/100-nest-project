...
export class PromptOptimizationService {
  ...
  async optimizePrompt(
    ...
    try {
      ...
      // 选择合适的优化模板
      const template = this.getOptimizationTemplate(
      ...
      // 调用AI进行优化
      const optimizedContent = await this.moonshotService.generateCompletion(
        optimizationPrompt,
        {
      ...
      // 分析原始提示词质量
      const originalAnalysis = this.analyzePrompt(request.prompt);

      // 分析优化后提示词质量
      const optimizedAnalysis = this.analyzePrompt(
        parsedResult.optimizedPrompt,
      );
  ...
  async batchOptimize(
    ...
    for (const promptText of prompts) {
      try {
        ...
        this.logger.error(
          `批量优化失败，提示词: ${promptText.substring(0, 50)}...`,
          error,
        );
        // 继续处理其他提示词
      }
  ...
  private getOptimizationTemplate(
  ...
  private buildOptimizationPrompt(
    ...
    let prompt = template.replace('{originalPrompt}', request.prompt);
  ...
  private parseOptimizationResult(content: string): {
    ...
    // 解析AI返回的结构化内容
    const optimizedPromptMatch = content.match(
      /### 优化后的提示词：\s*([\s\S]*?)(?=###|$)/,
    );
