  ...
  async referralUser(userId: number, referralDto: ReferralDto) {
    try {
      ...
      const existingReferral =
        await this.databaseService.userReferral.findUnique({
          where: { userId },
        });
    ...
    try {
      ...
      const results = await Promise.all(
        [...mergedRefCodes].map(async (refCode) => {
          ...
          // 获取该引荐码关联的所有用户ID
          const referredUsers =
            await this.databaseService.userReferral.findMany({
              where: {
                OR: [
                  { refCode: refCode },
          ...
          return {
            ...
            orderRate:
              totalUsers > 0
                ? Math.round((orderedCount / totalUsers) * 100) / 100
                : 0,
          };
        }),
      );
      ...
      return {
        ...
        data: {
          ...
          totalOrdered: results.reduce(
            (sum, item) => sum + item.orderedUsers,
            0,
          ),
        },
      };
  ...
  async createReferralCode(createReferralCodeDto: CreateReferralCodeDto) {
    try {
      ...
      if (!createReferralCodeDto.refCode) {
        throw new HttpException('引荐码不能为空', HttpStatus.BAD_REQUEST);
      }
    ...
    try {
      ...
      const referralCodesWithStats = await Promise.all(
        referralCodes.map(async (code) => {
          ...
          const orderedUsers = bindUserIds.filter((userId) =>
          ...
          const orderRate =
            totalUsers > 0
              ? Math.round((orderedUsers / totalUsers) * 100) / 100
              : 0;
      ...
      return {
        success: true,
        data:
          referralCodesWithStats?.map((item: any) => {
