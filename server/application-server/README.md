...
- 智能聊天助手功能:
  ...
  - **2025年LangGraphJS优化版本**:
    - **智能上下文压缩**: 基于LangGraphJS最新最佳实践的消息修剪策略
      - 动态消息数量计算：根据Agent类型和查询复杂度智能调整上下文窗口大小
      - 消息序列完整性保证：确保人类-AI消息配对的完整性，避免断片对话
      - Sliding Window策略：保留最近的重要对话轮次
      - 系统消息保护：自动保留和增强系统提示信息
    - **RAG检索增强生成**: 
      - **PostgreSQL向量数据库集成**：使用pgvector扩展进行高效语义检索
      - **智能文档检索**：基于用户查询的余弦相似度搜索
      - **多源检索策略**：同时检索向量知识库和聊天历史上下文
      - **相关性评分和过滤**：支持相似度阈值过滤（默认0.7）
      - **动态上下文注入**：将检索内容智能融入系统消息
      - **嵌入向量缓存**：支持向量生成和缓存机制
      - **知识库管理**：预置客服、技术支持、预约等领域知识
    - **增强的消息处理**:
      - 改进的AI回复提取逻辑：智能查找最后一条有效AI回复
      - 详细的处理统计和元数据记录
      - 压缩和检索状态的透明化展示
      - 性能监控和调试信息增强
    - **上下文管理策略**:
      - 技术支持代理：30条消息上下文（需要更多技术细节）
      - 客服代理：25条消息上下文（平衡效率和完整性）
      - 预约代理：15条消息上下文（专注当前预约需求）
      - 查询复杂度自适应：长查询增加20%上下文窗口
    - **智能功能开关**:
      - RAG功能按Agent能力自动启用
      - 上下文压缩的智能降级策略
      - 错误恢复和容错机制
  - 技术特性:
    ...
    - 完整的错误处理和性能统计
  - **RAG向量数据库使用说明**:
    - **初始化知识库**: `npm run db:seed` (包含向量知识库种子数据)
    - **生成嵌入向量**: `npm run generate-embeddings` (为知识库生成向量)
    - **向量检索**: 支持余弦相似度搜索，自动检索相关文档
    - **知识库扩展**: 可通过`VectorKnowledge`表添加新的知识条目
    - **性能优化**: 支持嵌入向量缓存，避免重复计算
